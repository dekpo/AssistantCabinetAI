...
| Markdown rendering in an answer | Feasible and light; the answer is currently dropped into a `<p>` with `white-space: pre-wrap`, so `**bold**` and list markers reach the screen literally. Render it with a library that builds a React tree rather than injecting HTML, because model output carries extracted document text. `docs/CHAT-UX-ASSESSMENT.md` item 1 |
...
| Stop button during generation | Feasible, but not as a view-only change: Tauri v2's `invoke` has no abort and `gateway.rs` has no cancellation check, so flipping the button alone would hide the answer while the model kept generating. Needs a cancellation token in `AppState` honoured by the stream loop, and it must also cancel the embedding leg of `ask_with_sources`. `docs/CHAT-UX-ASSESSMENT.md` item 2 |
