...
## Generated OCR fixtures (Sprint 2.5)

`inbox/` also holds fictional images and image-only PDFs, generated by the same script. Producing a
bitmap with the standard library alone is not reasonable, so the generator takes a guarded dependency
on Pillow (`pip install pillow`) for this purpose only — it is a development tool, not product code, and
the outputs below are committed as binaries like the other fixtures:

| File | Stands in for |
| --- | --- |
| `2026-03-22_courrier-rhumatologie-scan.pdf` | A real image-only PDF: French text rendered to a bitmap and embedded, no text layer |
| `2026-03-24_compte-rendu-mixte.pdf` | Page 1 born-digital, page 2 an image — the mixed-document case |
| `2026-03-26_ordonnance-scan.jpg` / `.png` | The same rendered page as a standalone image file |
| `2026-03-28_illisible.png` | Uniform random noise. No recoverable text |

`2026-03-20_radiographie-scan.pdf` stays exactly as it was for Sprint 2a: no image at all, so it
remains the "nothing to read" case with its two existing assertions intact.
